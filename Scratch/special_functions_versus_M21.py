...
special_functions = [fla10_new, fla10_alt, supendem21, supendem23]

r_23 =  [[0, 0], [6, 9], [2, 5], [3, 4], [5, 2], [4, 3], [9, 6], [1, 1]]


funcs = special_functions
relation = r_23
...
for k in range(len(funcs)):
    for i in coordinates:
        for j in coordinates:
            print('if (int(fun) ==', k, '){')
            print('stroke', colors[funcs[k](i, j)])
...
for k in range(len(funcs)):
    for i in range(len(lin_cart_p)):
        print('if (int(fun) ==', k, '){')
        print('stroke', colors[funcs[k](lin_cart_p[i], lin_cart_q[i])])
...
for i in range(len(funcs)):
    ...
    print("if (mouseY >", i*60, "&& mouseY <=", i*60+60, "){ fun = map(mouseX, 0, 450,", i*3, ",", i*3+3, "); }")

print( 'fill(0)')
print('text(spec[int(fun)], 100, 580)')