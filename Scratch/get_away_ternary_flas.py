...
def get_away_ternary(funcs, relation, place):
        ...
        for i in range(len(matrix_p1_aux)):
            for j in range(len(matrix_q1_aux)):
                if [funcs[k](matrix_p1_aux[i], matrix_q1_aux[j]),
                    ...
                    funcs[k](matrix_p3_aux[i], matrix_q3_aux[j])] not in relation:
                    if len(get_away) < 1:
                        get_away.append([i, j])
                        print('if (int(fun) ==', k, '){')
                        print('stroke',
                              colors[funcs[k](matrix_p1_aux[get_away[0][0]], matrix_q1_aux[get_away[0][1]])])
                        print('line', matrix_p1[get_away[0][0]] + matrix_q1[get_away[0][1]])
                        print('}')

                        print('if (int(fun) ==', k, '){')
                        print('stroke', colors[funcs[k](matrix_p2_aux[get_away[0][0]], matrix_q2_aux[get_away[0][1]])])
                        print('line', matrix_p2[get_away[0][0]] + matrix_q2[get_away[0][1]])
                        print('}')

                        print('if (int(fun) ==', k, '){')
                        print('stroke', colors[
                            funcs[k](matrix_p3_aux[get_away[0][0]], matrix_q3_aux[get_away[0][1]])])
