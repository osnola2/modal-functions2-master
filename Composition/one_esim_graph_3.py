...
def one_esim_ter(relation):
    ...
    for i in un_mod:
        for j in relation:
            if [i(j[0]), i(j[1]), i(j[2])] not in relation:
...
#     if type(K[i][0]) == list:
#         if len(K[i][0]) == 3:
#             # print(i, one_esim_ter(K[i]))
#             print(i, list(map(lambda x: val_to_func(x).__name__, one_esim_ter(K[i]))))
