...
def one_esim_bin(relation):
    ...
    for i in un_mod:
        for j in relation:
            if [i(j[0]), i(j[1])] not in relation:
    ...
    # b and c are arranging the list in the alphabetical order 0, 6, 9, 1
...
#     if type(K[i][0]) == list:
#         if len(K[i][0]) == 2:
#             print(i, one_esim_bin(K[i]))
#             print(i, list(map(lambda x: val_to_func(x).__name__, one_esim_bin(K[i]))))
#             print(K[i])
